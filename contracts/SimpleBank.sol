...
contract SimpleBank {
    ...
    function withdraw(uint withdrawAmount) public returns (uint) {
      ...
      //    sender's balance
      bool _sent = payable(msg.sender).send(withdrawAmount);
      require(_sent, 'withdrawal failed');
      uint _userBalance = balances[msg.sender] -= withdrawAmount;
      // 3. Emit the appropriate event for this message
      emit LogWithdrawal(msg.sender, withdrawAmount, _userBalance);
      return _userBalance;
    }
}
